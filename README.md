...
2. Add `-javaagent:/path/to/stacktrace-decoroutinator-jvm-agent-2.6.6.jar` to your JVM startup arguments. The corresponding dependency is `dev.reformator.stacktracedecoroutinator:stacktrace-decoroutinator-jvm-agent:2.6.6`.
3. Add the dependency `dev.reformator.stacktracedecoroutinator:stacktrace-decoroutinator-jvm:2.6.6` and call `DecoroutinatorJvmApi.install()`.
